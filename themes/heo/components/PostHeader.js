      ...
      <style jsx>{`
        .coverdiv:after {
          ...
          box-shadow: 110px -130px 500px 100px
            ${isDarkMode ? '#8f64af' : '#dc71a5'} inset;
        }
      `}</style>

      <div
        className={`${isDarkMode ? 'bg-[#8f64af]' : 'bg-[#dc71a5]'} absolute top-0 w-full h-full py-10 flex justify-center items-center`}>
        {/* 文章背景图 */}
        <div
            ...
            {post.category && (
              <>
                <Link
                  ...
                  passHref
                  legacyBehavior>
                  <div className='cursor-pointer font-sm font-bold px-3 py-1 rounded-lg  hover:bg-white text-white bg-pink-400 dark:bg-violet-800 hover:text-violet-700 duration-200 '>
                    {post.category}
                  </div>
                </Link>
              </>
